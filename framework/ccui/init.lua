...
function ccui.loadWidget(filename)
    filename = filename .. "_" .. INT.getLocaleName()

    if LOAD_CCS_FILE == LOAD_CCS_FILE_BY_CSB then
        filename = filename .. ".csb"
        widget = ccs.GUIReader:getInstance():widgetFromBinaryFile(filename)
    elseif LOAD_CCS_FILE == LOAD_CCS_FILE_BY_EXPORTJSON then
        filename = filename .. ".json"
        widget = ccs.GUIReader:getInstance():widgetFromJsonFile(filename)
    elseif LOAD_CCS_FILE == LOAD_CCS_FILE_BY_EXPORTJSON then
        filename = filename .. ".ExportJson"
        widget = ccs.GUIReader:getInstance():widgetFromJsonFile(filename)
    end
...
function ccui.loadArmature( filename )
    local armature,name = nil,""

    if LOAD_ARMATURE == LOAD_ARMATURE_BY_CCS then
    local t = string.split(filename,"/")
    name = t[1]
local tempName = filename .. ".csb"
local fullpath = cc.getFullPath(tempName)
if cc.isFileExist(fullpath) then
    filename = filename .. ".csb"
else
    filename = filename .. ".ExportJson"
    assert(cc.isFileExist(cc.getFullPath(filename)))
end
        ccs.ArmatureDataManager:getInstance():addArmatureFileInfo(filename)
        armature = ccs.Armature:create(name)
    elseif LOAD_ARMATURE == LOAD_ARMATURE_BY_DRAGON_BONES then
        ...
        armature = ccs.Armature:create(name)
        armature._info = skeleton
    elseif LOAD_ARMATURE == LOAD_ARMATURE_BY_SPINE then
        local pngPath = filename.pngPath
        local jsonPath = filename.plistPath
        local atlasPath = filename.xmlPath

        armature = sp.SkeletonAnimation:create(jsonPath, atlasPath, 0.6)
    end

    assert(armature,"can't load armature")
