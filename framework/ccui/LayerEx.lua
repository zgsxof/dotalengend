...
function Layer:load(filename)
        ...
        if widget then
            ...
            self.data = UIData:new()
            self.lastData = UIData:new()
            --注册一个update,update的实现
...
function Layer:checkData()
        if self.data:empty() == false and self.data:compare(self.lastData) == false then
            self.lastData:clone(self.data)
            self:reload()
        end
end

function Layer:reload()
