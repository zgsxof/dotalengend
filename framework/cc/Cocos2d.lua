...
if target == OS_WINDOWS then
    cc.platform = "windows"
elseif target == OS_MAC then
    cc.platform = "mac"
elseif target == OS_ANDROID then
    cc.platform = "android"
elseif target == OS_IPHONE or target == OS_IPAD then
    cc.platform = "ios"
    if target == OS_IPHONE then
        cc.model = "iphone"
    else
        cc.model = "ipad"
    end
...
function cc.init()
    --[[by zhangggs
    local frameSize = sharedDirector:getOpenGLView():getFrameSize()

    local size = cc.size(640,960)--cc.size(960,640)
    local scaleX = frameSize.width / size.width
    local scaleY = frameSize.height / size.height
    local scale = 0
    if scaleX > scaleY then
        scale = scaleX / (frameSize.height / size.height)
    else
        scale = scaleY / (frameSize.width / size.width)
    end

    cclog("cc.init, x:%f, y:%f, scale:%f",scaleX, scaleY, scale)

    sharedDirector:getOpenGLView():setDesignResolutionSize(size.width * scale,size.height * scale,cc.ResolutionPolicy.NO_BORDER)
    ]]
